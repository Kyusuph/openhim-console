...
export function TransactionsCtrl(
  ...
  $scope.channelSearchQuery = "";
  $scope.clientSearchQuery = "";
  $scope.selectedChannelName = "";
  $scope.toggleChannelDropdown = function () {
    const element = document.getElementById("channelDropdown");
    if ($scope.showChannelDropdown) {
    ...
    $scope.showChannelDropdown = !$scope.showChannelDropdown;
  };

  $scope.toggleClientDropdown = function () {
    const element = document.getElementById("clientDropdown");
    ...
    $scope.showClientDropdown = !$scope.showClientDropdown;
  };

  $scope.channelSelected = function (channelName, channelId) {
    console.log(channelName, channelId);
    if (!channelId) {
      $scope.settings.filter.transaction.channel = "";
      $scope.selectedChannelName = "";
    } else {
      $scope.settings.filter.transaction.channel = channelId;
      $scope.settings.filter.transaction.selectedChannelName = channelName;
  ...
  $scope.clientSelected = function (clientName, clientId) {
    if (!clientId) {
      $scope.settings.filter.transaction.client = "";
      $scope.settings.filter.transaction.selectedClientName = "";
    } else {
      $scope.settings.filter.transaction.client = clientId;
      $scope.settings.filter.transaction.selectedClientName = clientName;
      $scope.applyFiltersToUrl();
    }
    const element = document.getElementById("clientDropdown");
    element.style.display = "none";
  ...
  if ($location.search().txSelectedChannelName) {
    $scope.settings.filter.transaction.selectedChannelName =
      $location.search().txSelectedChannelName;
  }
  ...
  if ($location.search().routeStatusCode) {
    $scope.settings.filter.route.statusCode =
      $location.search().routeStatusCode;
    $scope.advancedFilters.isCollapsed = false;
  }
  ...
  const refreshSuccess = function (transactions) {
    ...
    // details paging.
    const idList = transactions.map(function (tx) {
      return tx._id;
    });
  ...
  $scope.getSelectedChannelName = function () {
    if ($scope.settings.filter.transaction.channel) {
      return $scope.channels.find(
        (channel) => channel._id === $scope.settings.filter.transaction.channel
      )?.name;
    } else {
      return "Don't filter";
    }
  ...
  $scope.getSelectedClientName = function () {
    if ($scope.settings.filter.transaction.client) {
      return $scope.clients.find(
        (client) => client._id === $scope.settings.filter.transaction.client
      )?.name;
    } else {
      return "Don't filter";
    }
  ...
  $scope.applyFiltersToUrl = function () {
            ...
            if ($scope.settings.filter.transaction.propertyKey) {
              ...
              if ($scope.settings.filter.transaction.propertyValue) {
                ...
                if ($scope.settings.filter.transaction.method) {
                  ...
                  if ($scope.settings.filter.transaction.selectedChannelName) {
                    $location.search(
                      "txSelectedChannelName",
                      $scope.settings.filter.selectedChannelName
                      );
                    }

                    if ($scope.settings.filter.transaction.selectedClientName) {
                      $location.search(
                        "txSelectedClientName",
                        $scope.settings.filter.selectedClientName
                        );
                      }
    ...
    if (filterParamsBeforeClear === filterParamsAfterClear) {
      ...
      $scope.selectedChannelName = $scope.settings.filter.selectedChannelName;
    }
  };
